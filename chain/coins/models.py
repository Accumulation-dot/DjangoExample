...
class CoinRule(models.Model):
    ...
    tax = models.FloatField(verbose_name='出售的税', help_text='手续费 百分比', default=0)
    pay = models.BooleanField(verbose_name='是否需要支付', help_text='是否需要支付金额, 手动去控制', default=0)
    shop = models.FloatField(verbose_name='获取币的购物百分比', help_text='获取的币 多少需要进行购物, 最大是1最小是0', default=0)
...
def query_coin(user):
    coins = Record.objects.filter(user=user).annotate(sum=Sum("point")).first()
    if coins is None:
        return 0.0
    return coins.point
